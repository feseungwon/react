...
function Timer() {
  ...
  const startTimer = () => {time === 0 ? setTime(10) : null; setIsRunning(true);};
  ...
  useEffect(() => {if (isRunning) {const timer = setInterval(() => {setTime((prev) => prev - 1);}, 1000);

  return () => clearInterval(timer);}}, [isRunning]);

  useEffect(() => {time === 0 ? setIsRunning(false) : null;}, [time]);
